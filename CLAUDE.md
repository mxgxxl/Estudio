...
2. **Deuda menor:**
   ...
   - **Tests fósiles de Emergent (TRES)**: `backend/tests/test_studyapp_backend.py`, `test_iteration4_gemini_only.py` y `test_iteration3_batching_penalty.py` golpean un `BASE_URL` remoto (preview de Emergent) vía `requests` → fallan en local/CI sin servidor. Migrarlos a in-process (TestClient + mongomock, como el resto) o retirarlos. **La suite in-process (la buena) son ~127 tests en verde**; correrla con esos ficheros da falsos negativos. Excluirlos con `--ignore` de los tres.
