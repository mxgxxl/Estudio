...
- **Modos de estudio → dos ejes (reestructuración en curso; Fases 1-2 hechas en backend).** El estudio se parte en dos ejes independientes: **`selection`** (`all` | `errors` | `srs` | `favorites` — *qué preguntas*, único filtro de query en `quiz_start`) y **`behavior`** (`practice` | `exam` — *cómo se juega*: timer/penalización, hoy client-side). `Attempt` persiste **solo** `selection` y `behavior` (fuente única).
  - **Compat de ENTRADA (viva hasta Fase 3):** el frontend viejo aún envía `mode` en el request de `quiz_start`/`quiz_submit`; se mapea a los ejes con `_resolve_quiz_axes` (exam→(all,exam), practice→(all,practice), errors→(errors,practice), srs→(srs,practice), favorites→(favorites,practice)). Ese `mode` de request **no se persiste**. `/quiz/start` aún **emite** un `mode` derivado en su respuesta (`_derive_mode`), no consumido por el front. Toda la capa de compat de `mode` (**entrada + salida**) se retira **junta en Fase 3** (cuando el frontend deje de enviar/leer `mode`), no en dos momentos.
