...
- **Modos de estudio → dos ejes (reestructuración en curso, Fase 1/4 hecha en backend).** El estudio se parte en dos ejes independientes: **`selection`** (`all` | `errors` | `srs` | `favorites` — *qué preguntas*, único filtro de query en `quiz_start`) y **`behavior`** (`practice` | `exam` — *cómo se juega*: timer/penalización, hoy client-side). `quiz_start`/`quiz_submit` aceptan los ejes nuevos y, por compat durante el despliegue, también el `mode` viejo (helper `_resolve_quiz_axes`; mapeo exam→(all,exam), practice→(all,practice), errors→(errors,practice), srs→(srs,practice), favorites→(favorites,practice)). `Attempt` persiste `selection` y `behavior`.
  ...
  - ⚠️ **`mode` es un campo PUENTE derivado y DEPRECADO**, no la fuente de verdad. Se escribe (`_derive_mode`, *lossy* a propósito) solo para no romper el label de "intentos recientes" de stats (`Stats.jsx` lee `attempt.mode`) hasta la **Fase 2**, que migra el histórico y la lectura de stats a los ejes y **retira `mode`**. **Nada nuevo debe apoyarse en `mode`**; usar `selection`/`behavior`. (La UI de dos ejes es Fase 3; el grueso de esta doc irá al cerrar la Fase 3.)
- **Dejar preguntas en blanco (examen).** El scoring (backend `quiz_submit`) trata la no respondida (`selected == -1`) como **neutra**: cuenta como `unanswered`, **no** como fallo, y **no penaliza** bajo ningún ratio (solo cuenta en el denominador de la nota). UX en `QuizRun.jsx`: botón **"Dejar en blanco"** siempre disponible en examen (con coletilla "(no penaliza)" **solo** si hay penalización), chip **"Sin responder"** en la pregunta actual y contador **"· N en blanco"** en el pie; al **Finalizar** con blancos, `window.confirm` "Te quedan N preguntas sin responder…" (solo examen; el auto-envío por tiempo agotado no pregunta; práctica no aplica). Resultados (`QuizResults`) ya muestra la blanca como estado propio "sin responder", no como fallo.
